...
关键词索引对中英文分别处理：英文、数字、下划线整体成词，中文按二元切分。二元切分不依赖词典，年报中的专有名词（如“云辇”）即使不在任何词表中也能被召回；代价是会切出跨词边界的噪声词元（如“的营”“入是”）。

查询侧因此按文档频率剪枝：文档频率超过语料子块总数 2% 的词元视为噪声，不参与召回，排序完全交给 BM25。阈值取自语料自身的统计量，不依赖任何领域词表。小语料上 2% 会退化成个位数，故另设绝对下限（10 个子块）。若一条查询的词元被全部剪掉，则退回未剪枝的词元，保证召回不会比朴素二元切分更差。

词元文档频率在写入时增量维护，覆盖写入同一子块会先撤销旧文本的贡献；执行上面的重建命令时统计随索引一并重建，无需额外操作。

在引入查询侧剪枝之前建立的索引没有统计表，剪枝会失效（检索仍可用，只是退化成朴素二元切分），此时检索日志会给出告警。补齐统计不需要访问 Milvus，直接从索引里已存的子块文本推导即可：
